...
from models.assl.assl import ASSL
...
def main(args):
    ...
    else:     #执行该语句，main_work(0,1,args)
        main_worker(args.gpu, 1, args)




def main_worker(gpu, ngpus_per_node, args):   # （0，1，args）
    '''
    ...
    # SET substitute model
    args.bn_momentum = 1.0 - args.ema_m  # = 0.001
    ...
    _eval_dset = SSL_Dataset(name=args.dataset, train=False,
                             num_classes=args.num_classes, data_dir=args.data_dir)
    eval_dset = _eval_dset.get_dset(eval_cifar=False)             #get valid dataset

    _eval_dset_cifar10 = SSL_Dataset(name=args.dataset, train=False,
                                     num_classes=args.num_classes, data_dir=args.data_dir)
    eval_dset_cifar10 = _eval_dset_cifar10.get_dset(eval_cifar=True)            #get test dataset
    ...
    # START TRAINING of ASSL
    for epoch in range(args.epoch):

       if epoch == 0 :
            print(".............", epoch, "...............")

            trainer = model.first_train

            model.set_data_loader(loader_dict)

            trainer(args, logger=logger)

            temp_lb_dset, temp_ulb_dset = train_dset.get_ssl_dset(args.num_labels,ask_thief_model=True,ask_blackbox_num=2166, uncertainty_choise=False ,user_10_argument=False)

            # update label and unlabel
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_name', type=str, default='cifar10-40')
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--load_path', type=str, default=None)
    parser.add_argument('--overwrite', default= True,action='store_true')
    
    '''
    Training Configuration of ASSL
    '''
    
    parser.add_argument('--epoch', type=int, default=10)
    parser.add_argument('--num_train_iter', type=int, default=70000,     # = epoch * every_num_train_iter
                        help='total number of training iterations')
    parser.add_argument('--every_num_train_iter', type=int, default=7000,
                        help='total number of training iterations')
    parser.add_argument('--num_eval_iter', type=int, default=1000,
                        help='evaluation frequency')
    parser.add_argument('--num_labels', type=int, default=1460,
                        help='Initial label sample size')
